  ...
  const handleBoostClick = () => {
    if (user.crystals < 5) {
      toast({
        ...
        description: "You need 5 crystals to boost your energy.",
        variant: "destructive",
      })
      return
    }
    ...
    updateUser({
      ...
      crystals: user.crystals - 5,
    })
    setTimeout(() => setIsAnimating(false), 1000)

    toast({
      title: "Energy Boosted",
      ...
      <div className="flex-grow space-y-2">
        <div className="flex justify-between text-lg font-bold">
          ...
          <span className="text-white">
            <span className={percentage < 30 ? "text-red-500" : "text-blue-400"}>{user.energy}</span> / {user.maxEnergy}
          </span>
        </div>
