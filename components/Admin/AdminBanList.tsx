...
import { faUnlock, faLock, faSearch } from "@fortawesome/free-solid-svg-icons"
...
export default function AdminBanList() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Add a "banned" property for demonstration
      const usersWithBanStatus = (data || []).map((user) => ({
        ...user,
        banned: false, // In a real implementation, this would come from the database
      }))
  ...
  const handleBanToggle = async (userId: number, currentBanStatus: boolean) => {
    ...
    setUsers(users.map((user) => (user.id === userId ? { ...user, banned: !currentBanStatus } : user)))
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-700">
                ...
                <tbody className="divide-y divide-gray-700">
                  {users.map((user) => (
                    <tr key={user.id} className={user.banned ? "bg-red-900/20" : ""}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${
                            user.banned ? "bg-red-900 text-red-200" : "bg-green-900 text-green-200"
                          }`}
                        >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 text-right">
                        ...
                        >
                          <FontAwesomeIcon icon={user.banned ? faUnlock : faLock} className="mr-2" />
